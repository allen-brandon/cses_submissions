...
class CentroidTree {
    ...
    CentroidTree(int n, SparseTable& table) : table(table), n(n) {
        ...
        fr(i,0,n) {
            partial_sum(dists[i].begin(), dists[i].end(), dists[i].begin());
            partial_sum(neg[i].begin(), neg[i].end(), neg[i].begin());
        }
    }
    ll query(int lb, int rb) {
        ...
        ll res = 0;
        fr(u,0,n) {
            res += dists[u].size()>rb ?  dists[u][rb] : dists[u].back();
            res -= dists[u].size()>lb ?  dists[u][lb] : dists[u].back();
            int prev = u;
            fr(i,0,anci[u]) {
                int c = anc[u][i];
                int d = ancd[u][i];
                int k = rb-d;
                if (k>=0) {
                    ll inc = dists[c].size() > k ? dists[c][k] : dists[c].back();
                    ll dec = neg[prev].size() > k ? neg[prev][k] : neg[prev].back();
                    res += inc - dec;
                }
                k = lb-d;
                if (k>=0) {
