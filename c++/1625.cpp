...
// _____ Sol With Hashing (tles) _____
// //1625
// using namespace std;
// #include <iostream>
// #include <vector>
...
// bool check(int i, int j, const auto& seen) {
//     if (i == 0 && j && !seen.contains(make_pair(i,j-1))) return 0;
//     if (i && j==6 && !seen.contains(make_pair(i-1,j))) return 0;
//     if (i == 6 && j<6 && !seen.contains(make_pair(i,j+1))) return 0;
//     if (i && j==0 && !seen.contains(make_pair(i-1,j))) return 0;
//     pair<int,int> p = make_pair(i,j);
//     if (p==make_pair(6,0)) return 0;
//     for (const auto [di, dj] : direction) {
//         int x=i+di;
//         int y=j+dj;
//         if (x<0 || x==7 or y<0 || y==7 || seen.contains({x,y})) continue;
        
//         int ind = 0;
//         for (const auto [dx, dy] : direction) {
//             int x1=x+dx;
//             int y1=y+dy;
//             ind += int(x1<0 || x1==7 or y1<0 || y1==7 || seen.contains({x1,y1}));
//         }
//         if (make_pair(x,y)==make_pair(6,0)) {
//             if (ind==4) {
//                 return 0;
//             }
//         } else if (ind==3) {
...
//         int x=i+di;
//         int y=j+dj;
//         if (x<0 || x==7 || y<0 || y==7 || seen.contains({x,y})) return;
//         st.emplace_back(x,y);
//         seen.emplace(x,y);
//         bt(s, res, st, seen);
//         st.pop_back();
...
//         int x=i+di;
//         int y=j+dj;
//         if (x<0 || x==7 || y<0 || y==7 || seen.contains({x,y})) continue;
//         st.emplace_back(x,y);
//         seen.emplace(x,y);
//         bt(s, res, st, seen);
//         st.pop_back();
